...
class PurchasesRepo:
    ...
    def get_returnable_for_items(self, purchase_id: str) -> list[dict]:
        ...
        For each purchase_items row in a purchase, return:
          - purchased_qty: SUM(quantity) for that item_id (effectively the line's quantity)
          - returned_qty : SUM(inventory_transactions.quantity) for returns referencing that item_id
          - remaining_returnable = purchased_qty - returned_qty

        Returns a list of dicts:
        [{ item_id, product_id, uom_id, purchased_qty, returned_qty, remaining_returnable }, ...]
        """
        sql = """
        SELECT
          pi.item_id,
    ...
    def _insert_header(self, h: PurchaseHeader):
        self.conn.execute("""
            INSERT INTO purchases(purchase_id, vendor_id, date, total_amount, order_discount,
                                  payment_status, paid_amount, advance_payment_applied, notes, created_by)
            VALUES (?,?,?,?,?,?,?,?,?,?)
        """, (h.purchase_id, h.vendor_id, h.date, h.total_amount,
              h.order_discount, h.payment_status, h.paid_amount,
              h.advance_payment_applied, h.notes, h.created_by))
    ...
    def _insert_inventory_purchase(self, *, item_id: int, product_id: int, uom_id: int, qty: float,
                                   pid: str, date: str, created_by: int | None, notes: str | None):
        # Legacy helper (unused by the new deterministic seq flow)
        self.conn.execute("""
    ...
    def create_purchase(self, header: PurchaseHeader, items: Iterable[PurchaseItem]):
        """
        - Recalculate totals from items (per-unit discount), minus order_discount.
        - Insert header with payment_status='unpaid', paid_amount=0, advance_payment_applied=0.
        - Insert purchase_items.
        - Insert inventory_transactions rows with transaction_type='purchase' and sequential txn_seq (10, 20, ...).
        """
        items_list = list(items)

        # --- 1) Recalculate totals (per-unit discount) ---
        order_disc = float(header.order_discount or 0.0)
        subtotal = 0.0
        for it in items_list:
            line_total = float(it.quantity) * (float(it.purchase_price) - float(it.item_discount))
            subtotal += line_total
        total_amount = max(0.0, subtotal - order_disc)

        with self.conn:
            # --- 2) Insert header with enforced payment fields ---
            self.conn.execute("""
            ...
            # Determine starting txn_seq for this date (10,20,...)
            ...
            # --- 3) Insert items + 4) Insert inventory with sequential txn_seq ---
            for it in items_list:
                ...
                cur = self.conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    header.purchase_id, it.product_id, it.quantity, it.uom_id,
                    it.purchase_price, it.sale_price, it.item_discount
                ))
                item_id = int(cur.lastrowid)

                # Inventory row for this item (explicit txn_seq)
                self.conn.execute("""
    ...
    def update_purchase(self, header: PurchaseHeader, items: Iterable[PurchaseItem]):
        ...
        Modify behavior:
          ...
          - Update header fields ONLY: vendor_id, date, order_discount, notes, total_amount.
          - Delete ONLY inventory_transactions for this purchase with transaction_type='purchase'.
          - Delete and re-insert purchase_items.
          - Re-insert corresponding inventory_transactions for purchase lines with new sequential txn_seq.
          - Do NOT touch existing 'purchase_return' rows.
        """
        items_list = list(items)
        ...
        with self.conn:
            # 1) Update header (only specific fields)
            self.conn.execute(
                """
                UPDATE purchases
                   SET vendor_id=?,
            ...
            # 2) Remove ONLY the purchase-line inventory rows (keep returns)
            self.conn.execute(
                """
            ...
            # 4) Determine next txn_seq for the (possibly new) date
            row = self.conn.execute(
                "SELECT COALESCE(MAX(txn_seq), 0) AS max_seq FROM inventory_transactions WHERE date = ?",
                (header.date,),
            ).fetchone()
            ...
            # 5) Re-insert items and corresponding inventory rows
            for it in items_list:
                it.purchase_id = header.purchase_id
                cur = self.conn.execute(
                    """
    ...
    def record_return(
        ...
        *,
        ...
        Enhanced returns:
          ...
          - Settlement:
              * {'mode':'refund', ...method & instrument fields...} => create negative purchase_payment
              * {'mode':'credit_note'} => create vendor credit (return_credit)
        """
        if not lines:
            return

        # Fetch header to get vendor_id
        ...
        # Insert return rows with high-bucket txn_seq (100, 110, ...)
        with self.conn:
            ...
            for ln in lines:
                # Safety: ensure product/uom still matches the purchase item
            ...
            if settlement and return_value > 0:
                ...
                if mode in ("refund", "refund_now"):
                    # Negative amount => refund from vendor (money coming in)
    ...
    # ---------- Vendor-scoped listings & summaries for statements ----------
    def list_purchases_by_vendor(
        ...
        """
        Purchases for a vendor with computed remaining_due.
        Fields:
          purchase_id, date, total_amount, order_discount, paid_amount,
          advance_payment_applied, payment_status, notes, remaining_due
        Ordered by DATE(date) ASC, purchase_id ASC.
        """
        sql = [
            """
            SELECT
              ...
              (CAST(p.total_amount AS REAL)
               ...
               - CAST(p.advance_payment_applied AS REAL)) AS remaining_due
            FROM purchases p
            WHERE p.vendor_id = ?
            """
        ]
    ...
    def get_purchase_totals_for_vendor(
        ...
        """
        Aggregate totals for a vendor across (optional) date range.
        Returns: {
          'purchases_total': float,
          'paid_total': float,
          'advance_applied_total': float
        }
        """
        sql = [
            """
    ...
    def list_return_values_by_purchase(self, purchase_id: str) -> list[dict]:
        """
        Optional helper: break down return valuations for a purchase.
        Reads from view purchase_return_valuations (schema-provided).
        Expected columns: transaction_id, item_id, qty_returned, unit_buy_price, unit_discount, return_value.
        """
        sql = """
        SELECT
          transaction_id,
    ...
    # ---------- (A) Removed legacy money mutation ----------
    # NOTE: The old apply_payment() header updater has been deprecated and removed.
    # All cash flows go through PurchasePaymentsRepo.record_payment(), and header
    # money fields are rolled up by DB triggers.
