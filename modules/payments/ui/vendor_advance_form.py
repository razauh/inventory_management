...
def open_vendor_advance_form(vendor_id: int, defaults: Optional[dict] = None) -> Optional[dict]:
    """
    Show a modal dialog to record a VENDOR ADVANCE / CREDIT (positive amount).
    ...
    if app is None:
        app = QApplication([])
        owns_app = True
    dlg = VendorAdvanceForm(parent=None, vendor_id=vendor_id, defaults=defaults or {})
    result = dlg.exec()
...
class VendorAdvanceForm(QDialog):
    """Dialog for recording vendor advances (credits).
    ...
    def __init__(self, parent: Optional[QWidget] = None, vendor_id: Optional[int] = None, defaults: Optional[dict] = None):  # type: ignore[override]
        super().__init__(parent)
        self.setWindowTitle(_t("Record Vendor Advance (Credit)"))
        self.setModal(True)
        ...
        # Optional adapters
        self._get_vendor_balance: Optional[Callable[[int], float]] = self._defaults.get("get_vendor_balance")
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel(_t("Record Vendor Advance (Credit)"))
        title.setObjectName("dlgTitle")
        title.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        outer.addWidget(title)

        # Vendor label
        vendor_text = str(self._vendor_display or (f"#{self._vendor_id}" if self._vendor_id is not None else ""))
        self.vendorLabel = QLabel(_t("Vendor: ") + vendor_text)
        outer.addWidget(self.vendorLabel)
        ...
        self.amountSpin.setRange(0.00, 1_000_000_000.00)  # validation enforces > 0 on save
        ...
        # Date controls (with option to leave empty → DB default CURRENT_DATE)
        ...
        # Hint under amount
        self.hintLabel = QLabel(_t("Posts store credit to the vendor’s ledger (positive amount). Apply to purchases from the Purchases screen."))
    ...
    def _apply_prefills(self) -> None:
        ...
        if isinstance(self._prefill_date, str):
            # Prefer explicit provided date, and uncheck 'no date'
            self.noDateCheck.setChecked(False)
            try:
    ...
    def _refresh_balance(self) -> None:
        if self._get_vendor_balance and isinstance(self._vendor_id, int):
            try:
                bal = float(self._get_vendor_balance(self._vendor_id))
                self.balanceLabel.setText(_t(f"Current Vendor Credit Balance: {bal:.2f}"))
    ...
    def _validate(self) -> tuple[bool, Optional[str]]:
        # 1) Parse amount
        # Even though QDoubleSpinBox ensures numeric text, we mimic controller phrasing.
        try:
            amt = float(self.amountSpin.value())
        except Exception:
            return False, _t("Enter a valid positive amount to grant as credit.")
        if amt <= 0.0:
